     ...
     def _check_email(self_list):
         for self in self_list :
             for mail in [self.email, self.personal_email]:
                 ...
                 email_list = self.sudo().search(['|', '|', ('email', '=ilike', mail), ('personal_email', '=ilike', mail), ('former_email_address', 'ilike', mail), ('is_company', '=', False), ('type', '=', 'contact'), ('active', '=', True),('user_id','=',False), ('employee_ids', '=', False)])
                 list_match = []
                 for e in email_list :
